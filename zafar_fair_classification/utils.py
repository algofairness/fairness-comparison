...
def print_classifier_fairness_stats(acc_arr, correlation_dict_arr, cov_dict_arr, s_attr_name):
    ...
    #TODO:: Check that this is a valid way of calculating CV score
    cv_score = (non_prot_pos - prot_pos)/100

    #TODO:: Check how to calculate Mutual Information!
    #mutual_information = non_prot_pos * np.ln(non_prot_pos/())
...
def print_mutual_information(all_class_labels_assigned_test, x_control_test, sensitive_attrs):
    """
    Calculating Mutual Information, a la Kamishima paper, now working for Zafar and Calder's code
    """

    #Could clean this line up, basically want the dictionary value corresponding to key of sensitive_attrs, in this case "sex"
    x_control = x_control_test[sensitive_attrs[0]]

    #Excessively switching back and forth between python lists and numpy arrays but working for now
    class_labels = np.array(all_class_labels_assigned_test)
    x_control = np.array(x_control)


    all_members =  float(len(x_control))                                        # Total number of data points
    non_prot_all = sum(x_control == 1.0)                                        # number of members in non-protected group
    prot_all = sum(x_control == 0.0)                                            # number of members in protected group
    positive_all = sum(class_labels == 1.0)                                     # Number of members in the positive class
    negative_all = sum(class_labels == -1.0)                                    # Number of members in the negative class
    non_prot_pos = sum(class_labels[x_control == 1.0] == 1.0)                   # Number of non_protected in positive class
    unprotected_negative = sum(class_labels[x_control == 1.0] == -1.0)          # Number of non_protected in negative class
    prot_pos = sum(class_labels[x_control == 0.0] == 1.0)                       # Number of protected in positive class
    protected_negative = sum(class_labels[x_control == 0.0] == -1.0)            # Number of protected in positive class



    probability_in_protected_group = prot_all/all_members                       # P(s_1) - Probability random individual in protected group
    probability_in_unprotected_group = non_prot_all/all_members                 # P(s_2) -Probability random individual in unprotected group


    probability_positive_all = positive_all/all_members                         # PR (y_1) --Probability random individual in positive class
    probability_negative_all = negative_all/all_members                         # PR (y_-1) --Probability random individual in negative class

    probability_protected_in_positive_class = prot_pos/all_members              #PR (s1, y1) -- Probability individual in both protected class and positive group
    probability_protected_in_negative_class = protected_negative/all_members    #PR (s1, y-1) -- Probability individual in both protected class and negative group
    probability_unprotected_in_positive_class = non_prot_pos/all_members        #PR (s2, y1) -- Probability indvidual in both unprotected class and positive group
    probability_unprotected_in_negative_class = unprotected_negative/all_members#PR (s2, y-1)-- Probability individual in both unprotected class and negative group


    PI = 0
    #CASE (S1, Y1)
    PI += probability_protected_in_positive_class*np.log2(probability_protected_in_positive_class/(probability_in_protected_group*probability_positive_all))
    #Case (S1, y-1)
    PI += probability_protected_in_negative_class*np.log2(probability_protected_in_negative_class/(probability_in_protected_group*probability_negative_all))
    #Case (S2, y1)
    PI += probability_unprotected_in_positive_class*np.log2(probability_unprotected_in_positive_class/(probability_in_unprotected_group*probability_positive_all))
    #Case (S2, y-1)
    PI += probability_unprotected_in_negative_class*np.log2(probability_unprotected_in_negative_class/(probability_in_unprotected_group*probability_negative_all))


    ProbY = [probability_positive_all, probability_negative_all]
    ProbS = [probability_in_protected_group, probability_in_unprotected_group]

    print "Prejudice Index (Mutual Information): %f" % PI
    z = scipy.stats.entropy(ProbS, base=2.0) * scipy.stats.entropy(ProbY, base=2.0)
    normalizedMI = PI/np.sqrt(z)
    print "Normalized Prejudice Index (normalized mutual information): %f" % normalizedMI


def compute_p_rule(x_control, class_labels):

    """ Compute the p-rule based on Doctrine of disparate impact """
    """ Also computing mutual information in here for now

    S is sensative variable, Y is target (predicted) variable
    !!    Mutual information should be non-negative, I believe!! """
...
def compute_cv_score(x_control, class_labels):
    ...
    frac_non_prot_pos = float(non_prot_pos) / float(non_prot_all)
    frac_prot_pos = float(prot_pos) / float(prot_all)
    cv_score = frac_non_prot_pos - frac_prot_pos
    print "CV score is: %f" % ( cv_score )
    return cv_score
