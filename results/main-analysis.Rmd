...
plot_tradeoff = function(measure, dataset) {
  ...
  var1 <- as.name(str_c(measure, "-numerical"))
  var2 <- as.name(str_c(measure, "-binsensitive"))
  ggplot(df, aes_q(x=var1, y=var2, colour=quote(algorithm))) + catscale20 + catscale20_2 +
...
make_sensitivity_figure = function(name) {
  ...
  plot_sensitivity(df, "DIavgall", "accuracy")
}
...
adult_race_params = read.csv("Feldman-SVM-DIavgall_adult_race_numerical-binsensitive.csv", stringsAsFactors = FALSE, check.names=TRUE) %>% 
...
```{r}
ricci_race_params = read.csv("ZafarFairness-DIavgall_ricci_Race_numerical-binsensitive.csv", stringsAsFactors = FALSE, check.names=FALSE) %>% 
...
# adult_m = compute_measure_correlation_matrix(adult_race_params, adult_measures)
# adult_m = (adult_m + t(adult_m))/2

# BUG: I'm having a hard time making RMarkDown save this to a file :( type this in your
# console instead...
#
# corrplot(adult_m, method="color", order="hclust", tl.cex=0.5)
# png(file="paper-figures/adult_measures_correlation.png")
```
