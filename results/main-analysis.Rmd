...
plot_tradeoff = function(measure, dataset) {
  ...
  var1 <- as.name(str_c(measure, "-numerical"))
  var2 <- as.name(str_c(measure, "-binsensitive"))
  ggplot(df, aes_q(x=var1, y=var2, colour=quote(algorithm))) + catscale + catscale2 +
...
print(p)
ggsave("paper-figures/preprocess-tradeoff-DIavgall.png", p)
```
...
algos_to_plot = c(
  "ZafarFairness", "Calders", "LR", "SVM", 
  "Feldman-SVM", "Feldman-LR", "DecisionTree", 
  "Feldman-DecisionTree", "Kamishima")

make_sensitivity_figure = function(name) {
  df = read.csv(name, check.names=TRUE) %>% filter(algorithm %in% algos_to_plot)
  plot_sensitivity(df, "accuracy", "DIavgall")
  }
