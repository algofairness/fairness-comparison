...
dataset = read.csv(str_c(dataset_name, "_", sensitive_attr, "_numerical-binsensitive.csv"), stringsAsFactors = FALSE, check.names=FALSE)
consistent_measures = c("DIbinary", "DIavgall", "CV", "TNR", "accuracy", "BCR", "MCC")

numeric_columns = sapply(dataset, class) == "numeric"
dataset_measures = subset(colnames(dataset), numeric_columns)

# ricci_measures
dataset_m = compute_measure_correlation_matrix(
 dataset, 
 c(consistent_measures, str_c(sensitive_attr, "-accuracy"), "0-accuracy", "1-accuracy", str_c(sensitive_attr, "-calibration+"),
   str_c(sensitive_attr, "-TNR"), str_c(sensitive_attr, "-calibration-"), str_c(sensitive_attr, "-TPR"), "TPR"))
...
corrplot(dataset_m, method="color", order="original")
```