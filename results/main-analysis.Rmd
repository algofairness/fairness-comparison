...
plot_tradeoff = function(measure, dataset) {
  ...
  var1 <- as.name(str_c(measure, "-numerical"))
  var2 <- as.name(str_c(measure, "-binsensitive"))
  ggplot(df %>% filter(!(algorithm %in% c("LR", "SVM", "GaussianNB", "DecisionTree"))), aes_q(x=var1, y=var2, colour=quote(algorithm))) + catscale10 + catscale10_2 + geom_abline(a=0,b=1, colour="gray") +
...
make_sensitivity_figure = function(name) {
  ...
  plot_sensitivity(df, "DIbinary", "accuracy")
}
