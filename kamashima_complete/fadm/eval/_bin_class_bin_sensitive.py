...
class BinClassBinSensitiveStats(BinClassStats):
    ...
    def p_rule(self):
        ...
        -------
        p_correct : float
            Pr[C=1 | S=0] / Pr[C=1 | S=1], p-rule on correct class
        p_estimated : float
            Pr[E=1 | S=0] / Pr[E=1 | S=1], p-rule on estimated class
        """

        c = np.sum(self.m, axis=2)
        e = np.sum(self.m, axis=1)

        p_correct = (c[0, 1] / (c[0, 0] + c[0, 1])) / (c[1, 1] / (c[1, 0] + c[1, 1]))
        p_estimated = (e[0, 1] / (e[0, 0] + e[0, 1])) / (e[1, 1] / (e[1, 0] + e[1, 1]))

        return p_correct*100, p_estimated*100


    def str_p_rule(self, header=True):
        """ Strings for p_rule()
        ...
        Returns
        -------
        p_rule : list, type=str
            list of message strings
        """

        p_correct, p_estimated = self.p_rule()

        pr = []
        if header:
            pr.append("### P-Score score ###")
        pr.append("P-Rule between Sensitive feature and correct class: Pr[C=0 | S=1] / Pr[C=1 | S=1] = %.15g" % (p_correct))
        pr.append("P-Rule between Sensitive feature and estimated class: Pr[E=0 | S=1] / Pr[E=1 | S=1] = %.15g" % (p_estimated))
