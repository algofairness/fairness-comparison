...
def load_adult_data(filename, load_data_size=None):
    ...
    """
    Only using numerical values from data, this might want to be changed in future versions
    """
    ...
    for line in open(filename):
        ...
        if len(line) != 7 or "?" in line: # if a line has missing attributes, ignore it
            continue
        if len(line) != 7 or "a" in line[0]: # if a line has missing attributes, ignore it
            continue
        class_label = line[-1]
        ...
        for i in range(0,len(line)-1):
            ...
            #This is left in from Zafar code in case these features are re-introduced
    ...
    def convert_attrs_to_ints(d): # discretize the string attributes
        for attr_name, attr_vals in d.items():
            #age,education_num,capital_gain,capital_loss,hours_per_week,sex,income_per_year
